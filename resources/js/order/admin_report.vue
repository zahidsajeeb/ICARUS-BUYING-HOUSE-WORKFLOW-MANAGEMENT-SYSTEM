<template>
    <div class="main-wrapper">
        <Header></Header>
        <Sidebar></Sidebar>
        <div class="page-wrapper">
            <div class="content container-fluid">
                <div class="page-header">
                    <div class="row">
                        <div class="col-sm-12">
                            <h3 class="page-title">ADMIN SECTION: (REPORT)</h3>
                            <hr>
                        </div>
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-12">
                        <div class="card" style="border:1px solid darkslategray !important;">
                            <div class="card-header" style="background:darkslategray !important; border-bottom:1px solid darkslategray !important;">
                                <h1 class="card-title mb-0" style="color:white; text-align:left; font-size:16px !important;">REPORT LIST:</h1>
                            </div>
                            <div class="card-body">
<!--                                <form enctype="multipart/form-data" @submit="formSubmit">-->
<!--                                    <div class="row" style="margin-bottom:10px !important;">-->
<!--                                        <div class="col-md-4">-->
<!--                                            <label class="col-form-label"><b>Date Start:</b></label>-->
<!--                                            <input type="text" class="form-control" placeholder="Enter Date . . .">-->
<!--                                        </div>-->
<!--                                        <div class="col-md-4">-->
<!--                                            <label class="col-form-label"><b>Date End:</b></label>-->
<!--                                            <input type="text" class="form-control" placeholder="Enter Date . . .">-->
<!--                                        </div>-->

<!--                                                                            <div class="col-md-4">-->
<!--                                                                                <select v-model="form.buyer" class="form-control">-->
<!--                                                                                    <option>-SELECT BUYER NAME-</option>-->
<!--                                                                                    <option v-for="(buyer, index) in buyers" :key="buyer.id" :value="buyer.id">{{ buyer.buyer_name }}</option>-->
<!--                                                                                </select>-->
<!--                                                                            </div>-->
<!--                                                                            <div class="col-md-4">-->
<!--                                                                                <select class="form-control">-->
<!--                                                                                    <option>-Select Buyer-</option>-->
<!--                                                                                    <option>OVS</option>-->
<!--                                                                                    <option>Matalan</option>-->
<!--                                                                                </select>-->
<!--                                                                            </div>-->
<!--                                        <div class="col-md-4">-->
<!--                                            <label class="col-form-label"> </label>-->
<!--                                            <a class="btn btn-success"> Submit</a>-->
<!--                                        </div>-->
<!--                                    </div>-->
<!--                                </form>-->
                                <div class="table-responsive">
                                    <table id="tableId" class="table table-stripped table-bordered table-striped">
                                        <thead>
                                        <tr>
                                            <th style="background:#0082f1 !important; color:white;">#</th>
<!--                                            <th style="background:#0082f1 !important; color:white;">User</th>-->
                                            <th style="background:#0082f1 !important; color:white;">Order <br> No</th>
                                            <th style="background:#0082f1 !important; color:white;">Buyer</th>
                                            <th style="background:#0082f1 !important; color:white;">Order Confirm <br> Date</th>
                                            <th style="background:#0082f1 !important; color:white;">FACTORY</th>
                                            <th style="background:#0082f1 !important; color:white;">L/C NUMBER</th>
                                            <th style="background:#0082f1 !important; color:white;">DESCRIPTION</th>
                                            <th style="background:#0082f1 !important; color:white;">SHIPMENT <br> DATE</th>
                                            <th style="background:#0082f1 !important; color:white;">SHIPMENT DATE <br> (AFTER EXT)</th>
                                            <th style="background:#0082f1 !important; color:white;">ORDER <br> QTY</th>
                                            <th style="background:#0082f1 !important; color:white;">PI TO <br> BUYER DATE</th>
                                            <th style="background:#0082f1 !important; color:white;">SC/LC TO <br> FTY DATE</th>
                                            <th style="background:#0082f1 !important; color:white;">SHELL FAB <br> (BOOKING DATE)</th>
                                            <th style="background:#0082f1 !important; color:white;">SHELL FAB <br> (PLAN DATE)</th>
                                            <th style="background:#0082f1 !important; color:white;">SHELL FAB <br> (ACTUAL)</th>
                                            <th style="background:#0082f1 !important; color:white;">SHELL FAB <br> (REMARKS)</th>
                                            <th style="background:#0082f1 !important; color:white;">POCKETING <br> (BOOKING DATE)</th>
                                            <th style="background:#0082f1 !important; color:white;">POCKETING <br> (PLAN DATE)</th>
                                            <th style="background:#0082f1 !important; color:white;">POCKETING <br> (ACTUAL)</th>
                                            <th style="background:#0082f1 !important; color:white;">POCKETING <br> (REMARKS)</th>
                                            <th style="background:#0082f1 !important; color:white;">FUSING</th>
                                            <th style="background:#0082f1 !important; color:white;">ZIPPER <br> (PLAN)</th>
                                            <th style="background:#0082f1 !important; color:white;">ZIPPER <br> (ACTUAL)</th>
                                            <th style="background:#0082f1 !important; color:white;">SWING THREAD <br> (PLAN)</th>
                                            <th style="background:#0082f1 !important; color:white;">SWING THREAD <br> (ACTUAL)</th>
                                            <th style="background:#0082f1 !important; color:white;">PRINT/EMB <br> (PLAN)</th>
                                            <th style="background:#0082f1 !important; color:white;">PRINT/EMB <br> (ACTUAL)</th>
                                            <th style="background:#0082f1 !important; color:white;">BUTTON</th>
                                            <th style="background:#0082f1 !important; color:white;">RIVET</th>
                                            <th style="background:#0082f1 !important; color:white;">MAIN <br> LABEL</th>
                                            <th style="background:#0082f1 !important; color:white;">SIZE <br> LABEL</th>
                                            <th style="background:#0082f1 !important; color:white;">CARE <br> LABEL</th>
                                            <th style="background:#0082f1 !important; color:white;">OTHER <br> LABEL</th>
                                            <th style="background:#0082f1 !important; color:white;">BACK PATCH/ <br> METAL PLAQUE</th>
                                            <th style="background:#0082f1 !important; color:white;">HANG TAG</th>
                                            <th style="background:#0082f1 !important; color:white;">STRETCH <br> TAG</th>
                                            <th style="background:#0082f1 !important; color:white;">WAIST <br> TAG</th>
                                            <th style="background:#0082f1 !important; color:white;">PRICE TAG/STICKER POLY <br> STICKER CTN  STICKER (PRICE)</th>
                                            <th style="background:#0082f1 !important; color:white;">PRICE TAG/STICKER POLY <br> STICKER CTN  STICKER (POLY)</th>
                                            <th style="background:#0082f1 !important; color:white;">PRICE TAG/STICKER POLY <br> STICKER CTN  STICKER (CTN)</th>
                                            <th style="background:#0082f1 !important; color:white;">HANGER/SIZER <br> (PLAN)</th>
                                            <th style="background:#0082f1 !important; color:white;">HANGER/SIZER <br> (ACTUAL)</th>
                                            <th style="background:#0082f1 !important; color:white;">SINGLE POLY/ <br> BLISTER POLY</th>
                                            <th style="background:#0082f1 !important; color:white;">CARTON</th>
                                            <th style="background:#0082f1 !important; color:white;">REMARKS <br> (TRIMS)</th>
                                            <th style="background:#0082f1 !important; color:white;">FIT SAMPLE <br> (PLAN)</th>
                                            <th style="background:#0082f1 !important; color:white;">FIT SAMPLE <br> (ACTUAL)</th>
                                            <th style="background:#0082f1 !important; color:white;">PP SAMPLE <br> (PLAN)</th>
                                            <th style="background:#0082f1 !important; color:white;">PP SAMPLE <br> (ACTUAL)</th>
                                            <th style="background:#0082f1 !important; color:white;">SHIPPING/TOP <br> SAMPLE (PLAN)</th>
                                            <th style="background:#0082f1 !important; color:white;">SHIPPING/TOP <br> SAMPLE (ACTUAL)</th>
                                            <th style="background:#0082f1 !important; color:white;">SIZE SET SWING/SWING <br> FINISH (START)</th>
                                            <th style="background:#0082f1 !important; color:white;">SIZE SET SWING/SWING <br> FINISH (FINISH)</th>
                                            <th style="background:#0082f1 !important; color:white;">SIZE SET WASH/WASH <br> FINISH (START)</th>
                                            <th style="background:#0082f1 !important; color:white;">SIZE SET WASH/WASH <br> FINISH (FINISH)</th>
                                            <th style="background:#0082f1 !important; color:white;">SWING INPUT DATE & <br> FINISH DATE (PLAN)</th>
                                            <th style="background:#0082f1 !important; color:white;">SWING INPUT DATE & <br> FINISH DATE (ACTUAL)</th>
                                            <th style="background:#0082f1 !important; color:white;">BULK WASH APP  & <br> FINISH DATE (PLAN)</th>
                                            <th style="background:#0082f1 !important; color:white;">BULK WASH APP  & <br> FINISH DATE (ACTUAL)</th>
                                            <th style="background:#0082f1 !important; color:white;">PACKING START & <br> FINISH DT (PLAN)</th>
                                            <th style="background:#0082f1 !important; color:white;">PACKING START & <br> FINISH DT (ACTUAL)</th>
                                            <th style="background:#0082f1 !important; color:white;">FINAL INSPECTION <br> DATE (PLAN)</th>
                                            <th style="background:#0082f1 !important; color:white;">FINAL INSPECTION <br> DATE (ACTUAL)</th>
                                            <th style="background:#0082f1 !important; color:white;">EX FTY DATE <br> (PLAN)</th>
                                            <th style="background:#0082f1 !important; color:white;">EX FTY DATE <br> (ACTUAL)</th>
                                            <th style="background:#0082f1 !important; color:white;">SHIP <br> QTY</th>
                                            <th style="background:#0082f1 !important; color:white;">REMARKS <br> (PRODUCTION)</th>
                                        </tr>
                                        </thead>
                                        <tbody>
                                        <tr v-for="(post, index) in posts" :key="post.id">
                                            <td class="text-center">{{ index + 1 }}.</td>
                                            <td>{{ post.order_no }}</td>
                                            <td>{{ post.buyer_name }}</td>
                                            <td>{{ post.order_confirm_date }}</td>
                                            <td>{{ post.factory }}</td>
                                            <td>{{ post.lc_no }}</td>
                                            <td>{{ post.description }}</td>
                                            <td>{{ post.shipment_date }}</td>
                                            <td>{{ post.shipment_date_after_ext }}</td>
                                            <td>{{ post.order_qty }}</td>
                                            <td>{{ post.pi_to_buyer_date }}</td>
                                            <td>{{ post.lc_to_fty_date }}</td>
                                            <td>{{ post.shell_fabric_booking_date }}</td>
                                            <td>{{ post.shell_fabric_plan_date }}</td>
                                            <td>{{ post.shell_fabric_actual }}</td>
                                            <td>{{ post.shell_fabric_remarks }}</td>
                                            <td>{{ post.pocketing_booking_date }}</td>
                                            <td>{{ post.pocketing_plan_date }}</td>
                                            <td>{{ post.pocketing_actual_date }}</td>
                                            <td>{{ post.pocketing_remarks }}</td>
                                            <td>{{ post.fusing }}</td>
                                            <td>{{ post.zipper_plan }}</td>
                                            <td>{{ post.zipper_actual }}</td>
                                            <td>{{ post.zipper_actual }}</td>
                                            <td>{{ post.swing_thread_actual }}</td>
                                            <td>{{ post.print_plan }}</td>
                                            <td>{{ post.print_actual }}</td>
                                            <td>{{ post.button }}</td>
                                            <td>{{ post.rivet }}</td>
                                            <td>{{ post.main_label }}</td>
                                            <td>{{ post.size_label }}</td>
                                            <td>{{ post.care_label }}</td>
                                            <td>{{ post.other_label }}</td>
                                            <td>{{ post.back_patch }}</td>
                                            <td>{{ post.hang_tag }}</td>
                                            <td>{{ post.stretch_tag }}</td>
                                            <td>{{ post.waist_tag }}</td>
                                            <td>{{ post.price }}</td>
                                            <td>{{ post.poly }}</td>
                                            <td>{{ post.ctn }}</td>
                                            <td>{{ post.hanger_plan }}</td>
                                            <td>{{ post.hanger_actual }}</td>
                                            <td>{{ post.single_poly }}</td>
                                            <td>{{ post.carton }}</td>
                                            <td>{{ post.trims_remarks }}</td>
                                            <td>{{ post.fit_sample_plan }}</td>
                                            <td>{{ post.fit_sample_actual }}</td>
                                            <td>{{ post.pp_sample_plan }}</td>
                                            <td>{{ post.pp_sample_actual }}</td>
                                            <td>{{ post.shipping_plan }}</td>
                                            <td>{{ post.shipping_actual }}</td>
                                            <td>{{ post.swing_start }}</td>
                                            <td>{{ post.swing_finish }}</td>
                                            <td>{{ post.wash_start }}</td>
                                            <td>{{ post.wash_finish }}</td>
                                            <td>{{ post.swing_input_plan }}</td>
                                            <td>{{ post.swing_input_actual }}</td>
                                            <td>{{ post.bulk_wash_plan }}</td>
                                            <td>{{ post.bulk_wash_actual }}</td>
                                            <td>{{ post.packing_start_plan }}</td>
                                            <td>{{ post.packing_start_actual }}</td>
                                            <td>{{ post.final_inspection_plan }}</td>
                                            <td>{{ post.final_inspection_actual }}</td>
                                            <td>{{ post.ex_fty_plan }}</td>
                                            <td>{{ post.ex_fty_actual }}</td>
                                            <td>{{ post.ship_qty }}</td>
                                            <td>{{ post.production_remarks }}</td>
                                        </tr>
                                        </tbody>
                                    </table>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>

            </div>
            <!-- /Page Content -->

        </div>
        <!-- /Page Wrapper -->

    </div>
    <!-- /Main Wrapper -->

    <!--    <div>Welcome {{ name }}</div>-->
</template>
<script>
import Sidebar from '../components/sidebar.vue';
import Header from '../components/header.vue';
import Footer from '../components/footer.vue';

export default {
    name: "admin_report",
    components: {Footer, Header, Sidebar},
    data() {
        return {
            form: {},
            posts: [],
            name: null,
        }
    },
    created() {
        if (window.Laravel.user) {
            this.name = window.Laravel.user.name
        }
        this.$axios.get('/sanctum/csrf-cookie').then(response => {
            this.$axios.get('/api/order/admin_report')
                .then(response => {
                    this.posts = response.data;
                    setTimeout(function () {
                            $('#tableId').DataTable(
                                {
                                    pagingType: 'full_numbers',
                                    pageLength: 10,
                                    processing: true,
                                    dom: 'Bfrtip',
                                    buttons: ['csv', 'print']
                                }
                            );
                        },
                        1000
                    )
                })
        }).catch(function (error) {
            console.log(error);
        });
    },
    // created() {
    //     if (window.Laravel.user) {
    //         this.name = window.Laravel.user.name
    //     }
    //     this.$axios.get('/sanctum/csrf-cookie').then(response => {
    //         this.$axios.get('/api/order/admin_report')
    //             .then(response => {
    //                 this.posts = response.data;
    //                 setTimeout(function () {
    //                         $('#tableId thead tr')
    //                             .clone(true)
    //                             .addClass('filters')
    //                             .appendTo('#tableId thead');
    //
    //                         var table = $('#tableId').DataTable({
    //                             columnDefs: [
    //                                 {
    //                                     searchable: false,
    //                                     orderable: false,
    //                                     targets: 0
    //                                 }
    //                             ],
    //                             order: [[1, 'asc']],
    //                             searchPane: true,
    //                             dom: '<"dt-buttons"Bf><"clear">lirtp',
    //                             initComplete: function () {
    //                                 var api = this.api();
    //                                 api
    //                                     .columns()
    //                                     .eq(0)
    //                                     .each(function (colIdx) {
    //                                         var cell = $('.filters th').eq(
    //                                             $(api.column(colIdx).header()).index()
    //                                         );
    //                                         var title = $(cell).text();
    //                                         $(cell).html('<input type="text" placeholder="' + title + '" />');
    //                                         $(
    //                                             'input',
    //                                             $('.filters th').eq($(api.column(colIdx).header()).index())
    //                                         )
    //                                             .off('keyup change')
    //                                             .on('change', function (e) {
    //                                                 $(this).attr('title', $(this).val());
    //                                                 var regexr = '({search})'; //$(this).parents('th').find('select').val();
    //                                                 var cursorPosition = this.selectionStart;
    //                                                 api
    //                                                     .column(colIdx)
    //                                                     .search(
    //                                                         this.value != ''
    //                                                             ? regexr.replace('{search}', '(((' + this.value + ')))')
    //                                                             : '',
    //                                                         this.value != '',
    //                                                         this.value == ''
    //                                                     )
    //                                                     .draw();
    //                                             })
    //                                             .on('keyup', function (e) {
    //                                                 e.stopPropagation();
    //
    //                                                 $(this).trigger('change');
    //                                                 $(this)
    //                                                     .focus()[0]
    //                                                     .setSelectionRange(cursorPosition, cursorPosition);
    //                                             });
    //                                     });
    //                             },
    //                             orderCellsTop: true,
    //                             fixedHeader: true,
    //                             paging: true,
    //                             autoWidth: true,
    //                             buttons: [
    //                                 {
    //                                     text: '<i class="fa fa-file-pdf"></i> Export PDF ',
    //                                     className: 'btn btn-danger',
    //                                     extend: 'pdfHtml5',
    //                                     filename: 'PDF REPORT',
    //                                     footer: true,
    //                                     orientation: 'landscape',
    //                                     pageSize: 'A4',
    //                                     exportOptions: {
    //                                         columns: ':visible',
    //                                         search: 'applied',
    //                                         order: 'applied',
    //                                         exportOptions: {}
    //                                     },
    //                                     customize: function (doc) {
    //                                         doc.content.splice(0, 1);
    //                                         doc.defaultStyle.alignment = 'center';
    //                                         var now = new Date();
    //                                         var jsDate = now.getDate() + '-' + (now.getMonth() + 1) + '-' + now.getFullYear();
    //                                         var logo = 'data:image/png;base64,';
    //                                         doc.pageMargins = [20, 60, 20, 30];
    //                                         doc.defaultStyle.fontSize = 9;
    //                                         doc.styles.tableHeader.fontSize = 9;
    //                                         doc['header'] = (function () {
    //                                             return {
    //                                                 columns: [
    //                                                     {
    //                                                         alignment: 'center',
    //                                                         text:
    //                                                             [
    //                                                                 {
    //                                                                     text: 'Peoples Oriented Program Implementation (POPI) ',
    //                                                                     bold: true
    //                                                                 }, '\n',
    //                                                                 {text: '(Item Recycle Report) ', alignment: 'center'},
    //                                                             ],
    //                                                         fontSize: 11,
    //                                                     },
    //                                                 ],
    //                                                 margin: 20
    //                                             }
    //                                         });
    //                                         doc['footer'] = (function (page, pages) {
    //                                             return {
    //                                                 columns: [
    //                                                     {
    //                                                         alignment: 'left',
    //                                                         // text: ['page ', { text: page.toString() },  ' of ', { text: pages.toString() }]
    //                                                         text: [{
    //                                                             text: 'This form/report is system generated',
    //                                                             bold: true,
    //                                                             border: true
    //                                                         }]
    //                                                     }
    //                                                 ],
    //                                                 margin: [10, 0]
    //                                             }
    //                                         });
    //                                         var objLayout = {};
    //                                         objLayout['hLineWidth'] = function (i) {
    //                                             return .5;
    //                                         };
    //                                         objLayout['vLineWidth'] = function (i) {
    //                                             return .5;
    //                                         };
    //                                         objLayout['hLineColor'] = function (i) {
    //                                             return '#aaa';
    //                                         };
    //                                         objLayout['vLineColor'] = function (i) {
    //                                             return '#aaa';
    //                                         };
    //                                         objLayout['paddingLeft'] = function (i) {
    //                                             return 4;
    //                                         };
    //                                         objLayout['paddingRight'] = function (i) {
    //                                             return 4;
    //                                         };
    //                                         doc.content[0].layout = objLayout;
    //                                         doc.content[0].table.widths = ['5%', '10%', '15%', '20%', '10%', '10%', '10%', '10%', '10%'];
    //                                     }
    //                                 }],
    //                             lengthMenu: [[10, 25, 50, 100, 500, -1], [10, 25, 50, 100, 500, "All"]],
    //                         });
    //                     }, 1000
    //                 )
    //             })
    //     })
    //     this.$axios.get(`/api/sample/buyer_list/`).then(response => {
    //         this.buyers = response.data;
    //     }).catch(function (error) {
    //         console.log(error);
    //     });
    // },
    methods: {
        logout(e) {
            e.preventDefault()
            this.$axios.get('/sanctum/csrf-cookie').then(response => {
                this.$axios.post('/api/logout')
                    .then(response => {
                        if (response.data.success) {
                            window.location.href = "/"
                        } else {
                            console.log(response);
                        }
                    })
                    .catch(function (error) {
                        console.error(error);
                    });
            })
        }
    },
    beforeRouteEnter(to, from, next) {
        if (!window.Laravel.isLoggedin) {
            window.location.href = "/";
        }
        next();
    }
}
</script>

